...
import { Plus, Search as SearchIcon, Edit, Trash2, Filter, Calendar } from 'lucide-react';
...
const SearchSection: React.FC = () => {
  ...
  const teamShops = useMemo(() => 
    shops.filter(shop => shop.team === activeTeam && shop.app === 'Search')
  , [shops, activeTeam]);
  ...
  const handleImportShops = async (data: any[]) => {
    return await importShops(data, activeTeam, 'Search', shops, agents, setShops);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Add/Edit Shop Modal */}
      <Modal isOpen={showModal} onClose={closeModal} title={editingShop ? t('shops.editSearch') : t('shops.addNewSearch')}>
