...
const DepositsSection: React.FC = () => {
  ...
  const filteredDeposits = useMemo(() => {
    let filtered = teamDeposits.filter(deposit =>
      deposit.shopId.toLowerCase().includes(searchTerm.toLowerCase()) ||
      deposit.agent.toLowerCase().includes(searchTerm.toLowerCase())
    );

    if (selectedAgent) {
      filtered = filtered.filter(deposit => deposit.agent === selectedAgent);
    }

    if (selectedApp) {
      filtered = filtered.filter(deposit => deposit.app === selectedApp);
    }

    if (selectedMonth) {
      filtered = filtered.filter(deposit => {
        const depositMonth = new Date(deposit.date).toISOString().slice(0, 7);
        return depositMonth === selectedMonth;
      });
    }

    return filtered;
  }, [teamDeposits, searchTerm, selectedAgent, selectedApp, selectedMonth]);
  ...
  const totalAmount = useMemo(() => 
    filteredDeposits.reduce((sum, deposit) => sum + deposit.amount, 0)
  , [filteredDeposits]);
  ...
  const handleDelete = (id: string) => {
    if (window.confirm(t('deposits.deleteConfirm'))) {
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      setSelectedItems(paginatedDeposits.map(deposit => deposit.id));
    } else {
      setSelectedItems([]);
    }
  };
  ...
  const handleExportDeposits = (month: string) => {
    let dataToExport = teamDeposits;
    
    if (month) {
      dataToExport = teamDeposits.filter(deposit => {
        const depositMonth = new Date(deposit.date).toISOString().slice(0, 7);
    ...
    const filename = `${activeTeam}_Deposits_${monthName.replace(/\s+/g, '_')}.csv`;
    
    exportToCSV(formattedData, filename);
  };

  const handleImportDeposits = async (data: any[]) => {
    return await importTransactions(data, activeTeam, 'deposit', shops, setTransactions);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-green-100">{t('deposits.totalDeposits')}</p>
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-blue-100">{t('deposits.totalAmount')}</p>
        ...
        <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-100">{t('deposits.averageDeposit')}</p>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-4 lg:space-y-0">
        <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4 w-full lg:w-auto">
          <div className="relative">
            ...
            <Input
              type="text"
              placeholder={t('deposits.searchDeposits')}
              value={searchTerm}
        ...
        <div className="flex flex-wrap gap-2">
          {selectedItems.length > 0 && (
            <Button variant="outline" onClick={handleDeleteSelected} className="flex items-center space-x-2">
              <Trash2 size={16} />
              <span>{t('deposits.deleteSelected')} ({selectedItems.length})</span>
            </Button>
          )}
          <ExportButton 
            onExport={handleExportDeposits}
            label={t('deposits.exportDeposits')}
            disabled={teamDeposits.length === 0}
          />
          <ImportButton 
            onImport={handleImportDeposits}
          ...
          <Button onClick={() => setShowModal(true)} className="flex items-center space-x-2">
            <Plus size={20} />
            <span>{t('deposits.addDeposit')}</span>
          </Button>
        </div>
      </div>
              ...
              <tr>
                <th className="px-6 py-3 text-left">
                  <input
                    ...
                    checked={paginatedDeposits.length > 0 && selectedItems.length === paginatedDeposits.length}
                ...
                paginatedDeposits.map((deposit) => (
                  <tr key={deposit.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="font-medium text-gray-900">{deposit.shopId}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {getAppBadge(deposit.app)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-gray-900">{deposit.agent}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
      ...
      <Modal isOpen={showModal} onClose={closeModal} title={editingTransaction ? t('deposits.editDeposit') : t('deposits.addNewDeposit')}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              required
              placeholder={t('transactions.depositAmount')}
            />
          </div>
          <div>
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <Button type="submit">
              {editingTransaction ? t('transactions.updateDeposit') : t('deposits.addDeposit')}
            </Button>
          </div>
        </form>
      </Modal>
